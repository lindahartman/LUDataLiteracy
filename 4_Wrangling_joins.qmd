...
:::{.challenge}
| Make a great table containing top-5 countries by GDP per capita from agriculture. Show also GPD per capita from other sources. Join in ISO country names. Which country names are not properly matched? Correct the country names in your table and show the country flags using `alpha3` code next to the country name.
| :bulb: Use gdp composition and existing gdp per capita columns in `cia_ecosec`
| :bulb: To manually correct the country names, use `.when().then(pl.lit('x')).when().then(pl.lit('y')).otherwise()` expression within `.with_columns()`. The `pl.lit()` is a wrapper for literal values (e.g. strings). 
:::
...
(
  ...
  .head(5)
  .with_columns(name=pl.when(pl.col('name')=="Faroe Islands")
