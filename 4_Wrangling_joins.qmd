...
:::{.challenge}
Given the GDP growth rate and population growth rate, what would be the average GDP per capital in every region next year? Show it as a column plot.
Is average GDP per capita in the Middle East is expected to be higher than the average GDP per capita in the East and Southeast Asia?
:::
...
(
  ...
  .group_by('region')
  .agg(pl.col('gdp_per_cap').mean())
  .pipe(ggplot)
  + geom_col(aes(x="region", y="gdp_per_cap"))
  + coord_flip()
)
```
