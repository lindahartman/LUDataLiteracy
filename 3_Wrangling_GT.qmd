...
| What are top-3 districts with the largest number of properties in energy class **B** or better? What are the number of missing values in the `energy_label` column of these district?
| :bulb: **B** or better requires info from both `energy_label` and `energy_eff`. 
...
| :bulb: For a binary variable, the proportion can be calculated with `mean()`
:::

```{python}
#LH: Suggest to skip - or postpone this
(
    ut
...
```{python}
#LH: I suggest to move this one be the first for Kings college
...
```{python}
# LH: I suggest to skip the outlier question (or help define outliers)
(
    kc
